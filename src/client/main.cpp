...
// 聊天客户端程序实现，mian线程用作发送线程，子线程用作接收线程
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 1:
        {
            ...
            {
                char buffer[2024] = {0};
                len = recv(clientfd, buffer, 2024, 0);
                if (-1 == len)
                {
                ...
                else
                {
                    ...
                    else // 登陆成功
                    {
                        ...
                        {
                            std::thread readTask(readTaskHandler, clientfd); // 底层相当于pthread_create
                            readTask.detach();
                            readthreadnumber++;
                        }
...
unordered_map<string, string> commandMap = {
    ...
    {"addgroup","加入群组，格式addgroup:group"},
